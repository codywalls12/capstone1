...
def graph_creation(request):
    ...
    # This for loop goes through the range of the y values entered by the user and adds them to the frequencies list
    ...
    # Once the melody list has been created, write it to the proper file as a .wav file
    ...
    if graph_type == 'bar':
        p = figure(x_range=x_values, height=figure_height, width=figure_width, title=graph_name, x_axis_label='X Values', y_axis_label='Y Values', tools="pan,box_zoom,wheel_zoom,reset,save", toolbar_location="right")
        p.vbar(x=x_values, top=y_values, width=0.8)  
    elif graph_type == 'scatter':
        p = figure(x_range=x_values, height=figure_height, width=figure_width, title=graph_name, x_axis_label='X Values', y_axis_label='Y Values', tools="pan,box_zoom,wheel_zoom,reset,save", toolbar_location="right")
        p.scatter(x_values, y_values, size=8)
